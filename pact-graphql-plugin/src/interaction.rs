...
pub use crate::graphql_payload::{GraphqlInlineSchema, GraphqlRequestPayload};
...
impl GraphqlInteractionBuilder {
    ...
    pub fn build(&self, req: GraphqlPluginRequest) -> anyhow::Result<GraphqlPluginConfig> {
        ...
        let mut canonical_schema = schema_sdl.map(|s| canonicalize_sdl(&s));
        if matches!(canonical_schema.as_deref(), Some(s) if s.is_empty()) {
        ...
        let (schema_ref, schema_inline_base64, inline_schema) = match canonical_schema {
            Some(canonical_sdl) => {
                let reference = self.registry.store(&canonical_sdl)?;
                let inline_base64 = BASE64_STANDARD.encode(canonical_sdl.as_bytes());
                let inline_schema = GraphqlInlineSchema {
                    base64_sdl: Some(inline_base64.clone()),
                };

                (Some(reference), Some(inline_base64), Some(inline_schema))
            }
            None => (None, None, None),
        };

        let request = GraphqlRequestPayload {
...
impl SchemaIndex {
    fn from_sdl(sdl: &str) -> anyhow::Result<Self> {
    ...
    fn root_type(&self, kind: OperationKind) -> Option<&str> {
        match kind {
...
fn canonicalize_variables(variables_json: Option<String>) -> anyhow::Result<Option<String>> {
...
fn canonicalize_sdl(input: &str) -> String {
